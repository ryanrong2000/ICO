...
contract ICO {
  ...
  constructor(
    ...
    uint _minPurchase,
    uint _maxPurchase) {
    token = ExampleToken(tokenAddress);
    require(_duration > 0, 'duration should be >0');
    require(_availableTokens > 0 && _availableTokens <= token.maxTotalSupply(),'_availableTokens should be >0 and <= maxTotalSupply');
  ...
  function buy(uint daiAmount) external icoActive(){
    require(daiAmount >= minPurchase && daiAmount <= maxPurchase, 'have to buy between minPurchase and maxPurchase');
  ...
  function widthdrawTokens() external icoEnded(){
    Sale storage sale = sales[msg.sender];
    require(sale.amount > 0, 'only investors');
    require(sale.tokensWithdrawn == false, 'tokens were already widthdrawn');
    sale.tokensWithdrawn = true;
    token.transfer(sale.investor, sale.amount);
  }

  function withdrawDai(uint amount) external onlyAdmin() icoEnded(){
